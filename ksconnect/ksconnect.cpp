...
namespace ksconnect
{
    void ksDetect::getksBar(cv::Mat &src)
    {
        ...
        threshold(img, img_thresholded, 110, 255, THRESH_BINARY); //190//110
    ...
    void ksDetect::Get(cv::Mat &img, tdtusart::Send_Struct_t &sendMessage)
    {
        ...
        for (const auto &contours : contours_poly)
        {
            ...
            const int Area_difference = 9000; //350//7000
            ...
            if ((contour.GetNum() == 6 && !isContourConvex(Mat(contours))))
            {
                ...
                if (abs(1 - contour.GetArea() / area * 0.56) < 0.56) //0.46
                {
                    l_condition2 = true;
                }
        ...
        for (auto &it : L_RU)
        {
            cv::putText(img, "ru", it.center_, cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 5, 8, 0);
            L1.push_back(L_RU.back().getCount());
        }
        for (auto &it : L_LD)
        {
            cv::putText(img, "ld", it.center_, cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 5, 8, 0);

            L2.push_back(L_LD.back().getCount());
        }
        for (auto &it : L_LU)
        {
            cv::putText(img, "lu", it.center_, cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 5, 8, 0);

            L3.push_back(L_LU.back().getCount());
        }
        for (auto &it : L_RD)
        {
            cv::putText(img, "rd", it.center_, cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 5, 8, 0);

            L4.push_back(L_RD.back().getCount());
        }
        for (auto &it : square)
        {
            cv::putText(img, "s", it.center_, cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 5, 8, 0);
                              ...
                              std::vector<engineer_tool::modelL> &L_RU, std::vector<engineer_tool::modelL> &square)
    {
        ...
        Mat imgThershold;
        threshold(img, imgThershold, 70, 255, THRESH_BINARY); //190
